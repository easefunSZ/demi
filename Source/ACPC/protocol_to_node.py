...
#= require "Game.card_to_string_conversion"
...
class ACPCProtocolToNode(object):
    ...
    # end
    def __init__(self):
        print('constructor of ACPCProtocolToNode')
    ...
    # -- @local
    def strstarts(self, string, start):
    ...
    def _parse_actions(self, actions):
        ...
        while actions_remainder != '':
            ...
            if string.starts(actions_remainder, "c"):
                ...
                actions_remainder = actions_remainder[: len(parsed_chunk)+ 1] )
        # end
        return out
    ...
    def _parse_state(self, state):
        ...
        _, _, position, hand_id, actions, cards = str.find(state, "^MATCHSTATE:(%d):(%d*):([^:]*):(.*)")
        ...
        # preflop_actions
        # flop_actions
        _, _, preflop_actions, flop_actions = str.find(actions, "([^/]*)/?([^/]*)")
        ...
        _, _, hand_p1, hand_p2, flop = str.find(cards, "([^|]*)|([^/]*)/?([^/]*)")
        print('hand_sb: %s', str(hand_sb))
        print('hand_bb: %s', str(hand_bb))
        print('flop: ', flop)
    ...
    def _compute_bets(self, processed_state):
        ...
        for i in range(1, len(processed_state.all_actions)):
        ...
        raise and i <= (  # processed_state.all_actions - 2):
    ...
    def _get_acting_player(self, processed_state):
        ...
        last_action = processed_state.all_actions[len(processed_state.all_actions)]
        ...
        if processed_state.current_street == 2 and len(
                processed_state.actions[2]) >= 2 and last_action.action == constants.acpc_actions.ccall:
            return -1
        # end
