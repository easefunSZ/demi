...
class ACPCProtocolToNode(object):
    ...
    def _parse_actions(self, actions):
        ...
        while actions_remainder != '':
            ...
            parsed_chunk = ''
            if string.starts(actions_remainder, "c"):
                table.insert(out, {action = constants.acpc_actions.ccall})
                parsed_chunk = "c"
            elif string.starts(actions_remainder, "r"):
                _
                raise_amount
                _, _, raise_amount = string.find(actions_remainder, "^r(%d*).*")
                raise_amount = tonumber(raise_amount)
                table.insert(out, {action = constants.acpc_actions.
                raise, raise_amount = raise_amount})
                parsed_chunk = "r" + raise_amount
                elif string.starts(actions_remainder, "f"):
                ...
                assert (  # parsed_chunk > 0)
                    actions_remainder = string.sub(actions_remainder, len(parsed_chunk + 1))
        ...
        def _parse_state(state)
            ...
            --MATCHSTATE:0:99:cc / r8146c / cc / cc:4
            cTs | Qs9s / 9
            h5d8d / 6
            c / 6
            d
...
list
actions, processed
with @ {_convert_actions_street} and
-- concatenated
-- @ local
...
-- * `position`: which
player
DeepStack is (element of @ {constants.players})
--
-- * `current_street`: the
current
betting
...
- -
-- * `acting_player`: which
player is acting(element
of @ {constants.players})
--
-- * `bet1`, `bet2`: the
number
of
...
def _process_parsed_state(parsed_state)
...
first_p1_action = {action = constants.acpc_actions.
raise, raise_amount = arguments.ante, player = constants.players.P1, street = 1}
first_p2_action = {action = constants.acpc_actions.
raise, raise_amount = arguments.ante, player = constants.players.P2, street = 2}

last_action = first_p1_action
prev_last_action = first_p2_action
...
if action.action == constants.acpc_actions.
raise and i <= (  # processed_state.all_actions - 2) then
...
if last_action.action == constants.acpc_actions.
raise and prev_last_action.action == constants.acpc_actions.
raise then
...
assert (prev_last_action.action == constants.acpc_actions.
raise and prev_last_action.raise_amount)
bet1 = prev_last_action.raise_amount
...
assert (last_action.action == constants.acpc_actions.
raise and last_action.raise_amount)
bet1 = prev_last_bet.raise_amount
bet2 = last_action.raise_amount
end
end
...
acting
player, as defined
by @ {constants.players}
-- @ local


def _get_acting_player(processed_state)
    ...
    last_action = processed_state.all_actions[  # processed_state.all_actions]
...
def parse_state(state)
    ...
    processed_state = self:_process_parsed_state(parsed_state)
...
def parsed_state_to_node(processed_state)
    node = {}
    ...
    node.bets = arguments.Tensor
    {processed_state.bet1, processed_state.bet2}
