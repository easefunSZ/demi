...
class LookaheadBuilder(object):
    ...
    def _construct_transition_boxes(self):
        ...
        def _compute_structure():
            ...
            self.lookahead.acting_player[1] = 1 - - in lookahead, 1
            does
            not stand
            for player IDs, it's just the first player to act
            for d=2, self.lookahead.depth+1:
        ...
        def construct_data_structures():
            ...
            for d=3, self.lookahead.depth:
                ...
                self.lookahead.average_strategies_data[d] = arguments.Tensor(self.lookahead.actions_count[d - 1],
                                                                             self.lookahead.bets_count[d - 2],
                                                                             self.lookahead.nonterminal_nonallin_nodes_count[
                                                                                 d - 2], game_settings.card_count):fill(
                    0)
        ...
        def set_datastructures_from_tree_dfs(node, layer, action_id, parent_id, gp_id):
            ...
            if layer < self.lookahead.depth + 1:
                ...
                if (not node.terminal) and (node.current_player != constants.players.chance):
                    ...
                    node_with_empty_actions = (  # node.children < self.lookahead.actions_count[layer])
                    ...
                    existing_bets_count =  # node.children - terminal_actions_count

                    ###--allin situations
                    if existing_bets_count == 0:
                        assert (action_id == self.lookahead.actions_count[layer - 1])

                    for child_id = 1, terminal_actions_count:
                    ...
                    for b = 1, existing_bets_count:
                        self.set_datastructures_from_tree_dfs(node.children[
                                                                  # node.children-b+1], layer+1, self.lookahead.actions_count[layer]-b+1, next_parent_id, next_gp_id)
        ...
        def _compute_tree_structures(current_layer, current_depth):
            ...
            self.lookahead.nonallinbets_count[
                current_depth] = layer_actions_count - layer_terminal_actions_count - 1 - -remove
            allin
            ###--if no alllin...
