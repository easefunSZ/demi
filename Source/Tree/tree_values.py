...
class TreeValues(object):
  ...
  def _fill_ranges_dfs(self, node, ranges_absolute):
    ...
    impossible_range_sum = node.ranges_absolute.clone().cmul(impossible_hands_mask:view(1, game_settings.card_count).expandAs(node.ranges_absolute)).sum()
  ...
  def compute_values(self, root, starting_ranges ):
    ...
    uniform_ranges = arguments.Tensor(constants.players_count, game_settings.card_count).fill(1.0/game_settings.card_count)
